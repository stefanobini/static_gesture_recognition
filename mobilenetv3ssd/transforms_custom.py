...
def get_dimensions_image(img: Tensor) -> List[int]:
...
class ResizeTo320(nn.Module):
    ...
    def forward(
        ...
        _, orig_height, orig_width = get_dimensions_image(image)
        new_width = 320
        new_height = 320
...
class FixedCenterSizeCrop(nn.Module):
    def __init__(self, size, fill=0, padding_mode="constant"):
        super().__init__()
        size = tuple(T._setup_size(size, error_msg="Please provide only two dimensions (h, w) for size."))
        self.crop_height = size[0]
...
class CenterEqualSizeCrop(nn.Module):
    ...
    def forward(self, img, target=None):
        ...
        if new_height != height or new_width != width:
            ...
            print(boxes[:, :])
            print("ENDD")
            globalxmin = min(boxes[:, 0])
            globalymin = min(boxes[:, 1])
            globalxmax = min(boxes[:, 2])
            globalymax = min(boxes[:, 3])
            ...
            tempymax = boxes[:, 3]
            print(tempxmin)
            print("difference xmin")
            print(tempxmin - left > 0)
            print("#######")
            print(tempymin)
            print("difference ymin")
            print(tempymin - top > 0)
            print("#######")
            print(tempxmax)
            print("difference xmax")
            print(tempxmax - left < crop_size)
            print(tempxmax - left)
            print("#######")
            print(tempymax)
            print("difference ymax")
            print(tempymax - top < crop_size)
            print(len(boxes))


            img, target = self._crop(img, target, top, left, new_height, new_width)

        pad_bottom = max(crop_size - new_height, 0)
        pad_right = max(crop_size - new_width, 0)
        if pad_bottom != 0 or pad_right != 0:
            img, target = self._pad(img, target, [0, 0, pad_right, pad_bottom])
